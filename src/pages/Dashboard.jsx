  ...
  return (
    <Card>
      <CardBody className="flex items-center gap-4">
        ...
        <div>
          <p className="text-sm text-primary-500 tracking-wide">{label}</p>
          <p className="text-2xl font-serif font-semibold text-primary-900">{value}</p>
        </div>
      </CardBody>
    </Card>
  );
...
export default function Dashboard() {
  ...
  const handleSave = form.handleSubmit((values) => {
    try {
      if (editing) {
        api.appointments.update(editing.id, values);
        toast.success('Appointment updated');
      } else {
        api.appointments.create(values);
    ...
    _actions: (
      <div className="flex gap-1">
        <button onClick={(e) => { e.stopPropagation(); handleSendReminder(a.id); }}
          className="p-1.5 rounded-lg hover:bg-accent-50 text-accent-600" title="Send reminder">
          <Send size={15} />
        </button>
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-serif font-semibold text-primary-900">Dashboard</h1>
          <p className="text-primary-500 mt-1 tracking-wide">Manage appointments and reminders</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard icon={Calendar} label="Today" value={todayCount} color="bg-accent-500" />
        <StatCard icon={Clock} label="Upcoming" value={upcomingCount} color="bg-secondary-500" />
        <StatCard icon={Users} label="Clients" value={clients?.length || 0} color="bg-primary-700" />
        <StatCard icon={MessageSquare} label="SMS Sent" value={smsStats?.sent || 0} color="bg-green-600" />
      </div>

      <Card>
        <CardHeader className="flex items-center justify-between">
          <h2 className="font-serif font-semibold text-primary-900 text-lg">Appointments</h2>
        </CardHeader>
        <Table columns={apptColumns} data={tableData} emptyMessage="No appointments yet — schedule your first one" />
      </Card>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editing ? 'Edit Appointment' : 'New Appointment'}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <Input
            ...
            onChange={form.handleChange}
            placeholder="e.g. Haircut, Beard Trim, Colour"
          />
